...
async def _async_migrate_network(call: ServiceCall) -> dict[str, Any]:
    ...
    async with async_get_dataset_lock(call.hass):
        ...
        # that "already on network" would be wrong in a few minutes. Nor is
        # it a no-op while a pending dataset is being replaced: the mesh is
        # about to leave this network, so re-targeting it is the point.
