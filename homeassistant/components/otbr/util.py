...
class OTBRData:
    ...
    async def set_pending_dataset_tlvs(self, dataset: bytes) -> None:
        ...
        more verdicts: a rejection -- it is not attached, or the leader
        refused the dataset -- and no verdict at all, when the leader did
        not answer in time. Each gets its own error, since the caller has
        to treat them differently: nothing happened, versus something may
        have.
        """
        try:
